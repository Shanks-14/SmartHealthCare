...
const Login = () => {
  ...
  // BUG FIX: Extracted core login logic into a separate async function that does NOT
  // expect an event object. The form's onSubmit handler wraps it and calls e.preventDefault().
  // Previously, demoLogin() called handleLogin() without an event, causing
  // "Cannot read properties of undefined (reading 'preventDefault')" crash.
  const performLogin = async (emailVal, passwordVal, roleVal) => {
    ...
    try {
      ...
      if (emailVal === 'admin@smartcare.com' && passwordVal === 'admin123') {
        navigate('/admin/dashboard');
      } else {
        switch (response.user?.role || roleVal) {
          case 'patient':
  ...
  // Form submit handler — receives the event and prevents default form submission
  const handleLogin = (e) => {
    e.preventDefault();
    performLogin(email, password, role);
  };

  // BUG FIX: demoLogin now calls performLogin() directly instead of handleLogin(),
  // so there is no crash from a missing event object
  const demoLogin = (demoRole) => {
    let demoEmail = '';
    let demoPassword = '';

    if (demoRole === 'patient') {
      demoEmail = 'sarah.murphy@gmail.com';
      demoPassword = 'SecurePass123!';
    } else if (demoRole === 'doctor') {
      demoEmail = 'dr.obrien@beaumont.ie';
      demoPassword = 'DoctorPass123!';
    } else if (demoRole === 'admin') {
      demoEmail = 'admin@smartcare.com';
    ...
    performLogin(demoEmail, demoPassword, demoRole);
  };
